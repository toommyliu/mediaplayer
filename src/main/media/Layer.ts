  ...
  Effect.gen(function* () {
    ...
    const workerPool = yield* Effect.acquireRelease(
      Effect.tryPromise({
        try: async () => {
          ...
          const pool = new WorkerPool(poolSize, fileWorkerPath);
    ...
    const buildFileTree = (
      ...
      Effect.gen(function* () {
        ...
        const subdirectoryResults = yield* Effect.all(
          ...
          videoFileTasks.length > 0
            ? yield* Effect.all(
                videoFileTasks.map((task) =>
                  Effect.tryPromise({
                    try: async () => await workerPool.processFile(task.path),
                    catch: (error) => error
                  }).pipe(
                    Effect.catch((error) => {
                      logger.error(`Error getting duration for ${task.path}`, error);
    ...
    const getAllVideoFilesRecursive = (
      ...
      Effect.gen(function* () {
        ...
        const scan = (dirPath: string): Effect.Effect<void, unknown> =>
          Effect.gen(function* () {
            const contents = yield* loadDirectoryContents(dirPath);
