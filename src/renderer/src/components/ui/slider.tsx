  ...
  return (
    ...
    >
      ...
      <SliderPrimitive.Control
        className="flex touch-none py-3 select-none data-disabled:pointer-events-none data-disabled:opacity-64 data-[orientation=horizontal]:w-full data-[orientation=vertical]:h-full data-[orientation=vertical]:flex-col"
      ...
      >
        ...
        >
          ...
          {markers?.map((marker, index) => {
            ...
            if (max <= min || timestamp < min || timestamp > max || !Number.isFinite(timestamp)) {
              return null;
            }

            const position = Math.min(100, Math.max(0, ((timestamp - min) / (max - min)) * 100));
