  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={(
          <div
            className="group/playlist-item flex h-9 min-w-0 items-center gap-2 rounded-md px-2 text-left transition-colors hover:bg-muted/50"
            onClick={onPlay}
            role="button"
            tabIndex={0}
          >
            <span className="w-4 shrink-0 text-center text-[0.625rem] font-medium tabular-nums text-muted-foreground/60">
              {index + 1}
            </span>
            ...
            <Button
              className="hidden size-5 p-0 text-muted-foreground/60 hover:bg-destructive/10 hover:text-destructive group-hover/playlist-item:inline-flex group-focus-within/playlist-item:inline-flex"
              onClick={(event) => {
                event.stopPropagation();
                onRemove();
              }}
...
export function PlaylistsPanel() {
  ...
  const removePlaylistItem = usePlaylistsStore(
    state => state.removePlaylistItem,
  );
  const renamePlaylist = usePlaylistsStore(state => state.renamePlaylist);
  const replacePlaylistItems = usePlaylistsStore(
    state => state.replacePlaylistItems,
  );
  ...
  const [renamingPlaylist, setRenamingPlaylist] = useState<Playlist | null>(
    null,
  );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 pt-1">
      ...
      {sortedPlaylists.length === 0
        ? (
            <div className="flex min-h-0 flex-1 items-center justify-center rounded-lg border border-dashed border-sidebar-border/60 px-4 py-8 text-center">
              <div className="max-w-48 space-y-3">
                <ListMusicIcon className="mx-auto size-8 text-muted-foreground/50" />
                <div>
        ...
        : (
            <>
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 truncate text-[0.625rem] text-muted-foreground/70">
                          ...
                          {" - "}
                          {formatPlaylistDate(
                            playlist.lastPlayedAt ?? playlist.updatedAt,
                          )}
                        </div>
                      </div>

                      <div className="flex shrink-0 items-center gap-0.5 opacity-0 transition-opacity group-hover/playlist:opacity-100 group-focus-visible/playlist:opacity-100 group-focus-within/playlist:opacity-100">
                        <PlaylistIconButton
                        ...
                        <DropdownMenu>
                          <Tooltip>
                            <TooltipTrigger
                              render={(
                                <DropdownMenuTrigger
                                  className="inline-flex size-6 items-center justify-center rounded-md text-muted-foreground/70 hover:bg-primary/10 hover:text-primary"
                          ...
                          <DropdownMenuContent align="end" className="min-w-44">
                            ...
                            <DropdownMenuSeparator />
                            <DropdownMenuItem
                              onClick={() => setRenamingPlaylist(playlist)}
                            >
                            ...
                            <DropdownMenuItem
                              ...
                              onClick={() =>
                                replacePlaylistItems(playlist.id, queueItems)}
                            >
              ...
              <div className="flex min-h-0 flex-1 flex-col border-t border-sidebar-border/60 pt-2">
                {selectedPlaylist
                  ? (
                      <>
                        <div className="flex shrink-0 items-start justify-between gap-2 pb-2">
                          <div className="min-w-0">
                            ...
                            <div className="mt-0.5 text-[0.625rem] text-muted-foreground/70">
                        ...
                        {selectedPlaylist.items.length === 0
                          ? (
                              <div className="flex min-h-0 flex-1 items-center justify-center rounded-md border border-dashed border-sidebar-border/60 px-4 py-8 text-center text-xs text-muted-foreground">
                                ...
                                <div className="flex flex-col gap-1 pr-1">
                                  {selectedPlaylist.items.map((item, index) => (
                                    <PlaylistItemRow
                                      ...
                                      onRemove={() =>
                                        removeSelectedPlaylistItem(item)}
                                    />
                                  ))}
                                </div>
                              </ScrollArea>
